...
from solara.ui import select, slider  # 正確 import 元件
...
def Page():
    ...
    country = select(label="Select country", options=country_options, value=country_options[0])

    # 滑動尺標：人口範圍
    min_pop = int(df["population"].min())
    max_pop = int(df["population"].max())
    population_range = slider(label="Population", min=min_pop, max=max_pop, value=(min_pop, max_pop))
